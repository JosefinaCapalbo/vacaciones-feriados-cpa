...
const PASS = "admin123";
const MAX_DEF = 10;
const GCAL_CLIENT_ID = "210407386870-9jqrj6ik89obfcqo8ms0p0pjoh9hcs65.apps.googleusercontent.com";
...
async function dbSave(state){try{await supabase.from("vacaciones_app").upsert({id:DB_KEY,data:JSON.stringify(state),updated_at:new Date().toISOString()});}catch(e){console.error("Error guardando:",e);}}
...
function CalendarioPublico({emps,datos}){
  ...
  return <div>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
      <button onClick={prev} style={{background:CARD,border:"1px solid "+BDR,borderRadius:10,padding:"10px 18px",cursor:"pointer",fontSize:20,color:TXT}}>â</button>
      ...
      <button onClick={next} style={{background:CARD,border:"1px solid "+BDR,borderRadius:10,padding:"10px 18px",cursor:"pointer",fontSize:20,color:TXT}}>â¶</button>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4,marginBottom:4}}>{["Dom","Lun","Mar","MiÃ©","Jue","Vie","SÃ¡b"].map(d=><div key={d} style={{textAlign:"center",fontSize:12,color:MUT,fontWeight:600,padding:"4px 0"}}>{d}</div>)}</div>
...
export default function App(){
  ...
  const [form,setForm]=useState({tipo:"vacaciones",ini:"",fin:""});
  ...
  function reset(){setForm({tipo:"vacaciones",ini:"",fin:""});setEditGid(null);setMsgOk("");setMsgErr("");}

  async function cargar(){
    setMsgOk("");setMsgErr("");
    const{tipo,ini:a,fin:b}=form;
    if(!a)return setMsgErr("IngresÃ¡ fecha de inicio.");
    if(tipo==="vacaciones"&&!b)return setMsgErr("IngresÃ¡ fecha de fin.");
    const finR=b||a;
    if(b&&b<a)return setMsgErr("Fin no puede ser anterior al inicio.");
    const segs=splitAnio(a,finR);
    if(!segs.length)return setMsgErr("Sin dÃ­as hÃ¡biles en el rango.");
    ...
    const gid=Date.now();
    const nuevos=segs.map((seg,i)=>({id:gid+i,grupoId:gid,tipo,ini:seg.ini,fin:seg.fin,dias:seg.dias,anio:seg.anio}));
    let regs=(datos[empSel]||[]);
    if(editGid)regs=regs.filter(r=>(r.grupoId||r.id)!==editGid);
    save(null,null,{...datos,[empSel]:[...regs,...nuevos]});
    const total=segs.reduce((a,s)=>a+s.dias,0),cruzaMsg=segs.length>1?" (cruza aÃ±o)":"";
    if(gcal.token){const titulo=(tipo==="vacaciones"?"ð´ Vacaciones - ":"ð Feriado - ")+empSel;const ok=await gcal.crearEvento(titulo,a,finR);setMsgOk("â "+total+"d registrado(s)"+cruzaMsg+(ok?" y sincronizado con Google Calendar ðï¸":" (error al crear evento en Calendar)"));}
    else{setMsgOk("â "+total+"d registrado(s)"+cruzaMsg);}
    reset();
  }
  ...
  if(vista==="login")return(<div style={page}><div style={{maxWidth:460,margin:"0 auto"}}>
    <div style={{background:"#f9e4d4",borderRadius:18,padding:"1.2rem",marginBottom:14,textAlign:"center"}}><div style={{fontSize:36}}>ð´</div><h1 style={{margin:"6px 0 0",fontSize:20,fontWeight:600,color:TXT}}>Vacaciones & Feriados</h1></div>
    <div style={{display:"flex",gap:6,marginBottom:14}}>{[["emp","ð¤ Ingresar"],["cal","ð Calendario"]].map(([t,l])=><button key={t} onClick={()=>setLoginTab(t)} style={{flex:1,background:loginTab===t?"#f7c5a8":CARD,border:"1px solid "+BDR,borderRadius:10,padding:"10px",cursor:"pointer",fontWeight:loginTab===t?600:400,color:TXT,fontSize:15}}>{l}</button>)}</div>
    {loginTab==="emp"&&<><Card title="Acceso empleado"><label style={{fontSize:14,color:MUT,display:"block",marginBottom:6}}>Tu nombre</label><select value={empSel} onChange={e=>setEmpSel(e.target.value)} style={inp}><option value="">-- Elegir --</option>{visibles.map(e=><option key={e}>{e}</option>)}</select><Btn onClick={()=>{if(empSel){setVista("emp");reset();}}} disabled={!empSel}>Ingresar</Btn></Card>
    <Card title="Acceso administrador"><label style={{fontSize:14,color:MUT,display:"block",marginBottom:6}}>ContraseÃ±a</label><input type="password" value={pass} placeholder="ContraseÃ±a" onChange={e=>{setPass(e.target.value);setPassErr(false);}} onKeyDown={e=>{if(e.key==="Enter"){if(pass===PASS){setVista("admin");setAdminEmp(emps[0]||"");}else setPassErr(true);}}} style={inp}/>{passErr&&<p style={{color:"#b94a4a",fontSize:13,margin:"4px 0"}}>ContraseÃ±a incorrecta.</p>}<Btn bg2="#c5b4e3" tc={TXT} onClick={()=>{if(pass===PASS){setVista("admin");setAdminEmp(emps[0]||"");}else setPassErr(true);}}>Ingresar como admin</Btn></Card></>}
  ...
  if(vista==="emp"){
    ...
    return <div style={page}><div style={{maxWidth:500,margin:"0 auto"}}>
      ...
      <div style={{background:gcal.status==="ok"?"#d4f0c0":CARD,border:"1px solid "+BDR,borderRadius:12,padding:"10px 14px",marginBottom:12,display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:20}}>ðï¸</span><div style={{flex:1}}><div style={{fontWeight:600,fontSize:13,color:TXT}}>Google Calendar</div><div style={{fontSize:12,color:MUT}}>{gcal.status==="ok"?"Conectado":gcal.status==="loading"?"Conectando...":gcal.status==="error"?"Error al conectar.":"ConectÃ¡ para sincronizar"}</div></div>{gcal.status!=="ok"&&<button onClick={gcal.conectar} disabled={gcal.status==="loading"} style={{background:"#4285f4",color:"#fff",border:"none",borderRadius:8,padding:"6px 12px",fontSize:12,fontWeight:600,cursor:"pointer"}}>{gcal.status==="loading"?"...":"Conectar"}</button>}{gcal.status==="ok"&&<span style={{color:"#2d6a1f",fontWeight:700}}>â</span>}</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}><Saldo label="Vacaciones" uso={uV} max={mV} emoji="ð´" color="#c5dff8" tc="#2a6496"/><Saldo label="Feriados" uso={uF} max={mF} emoji="ð" color="#d4f0c0" tc="#2d6a1f"/></div>
      <Card title={editGid?"âï¸ Editar registro":"Cargar dÃ­as"}>
        ...
        <Btn onClick={cargar}>{editGid?"Guardar cambios":"Registrar dÃ­as"}</Btn>
      </Card>
      {gs.length>0&&<Card title="Mi historial">
        ...
        {gs.filter(g=>g.segs.some(s=>s.anio===filtroAnio)).length===0?<p style={{color:MUT,fontSize:13}}>Sin registros en {filtroAnio}.</p>:gs.filter(g=>g.segs.some(s=>s.anio===filtroAnio)).map(g=>{const s0=g.segs[0],sN=g.segs[g.segs.length-1];return <div key={g.gid} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 0",borderBottom:"1px solid "+BDR}}><span style={{background:g.tipo==="vacaciones"?"#c5dff8":"#d4f0c0",color:g.tipo==="vacaciones"?"#2a6496":"#2d6a1f",borderRadius:8,padding:"3px 8px",fontSize:12,fontWeight:600,whiteSpace:"nowrap"}}>{g.tipo==="vacaciones"?"ð´ Vac.":"ð Fer."}</span><span style={{fontSize:13,color:TXT,flex:1}}>{fmt(s0.ini)}{s0.ini!==sN.fin?" â "+fmt(sN.fin):""}{g.segs.length>1&&<span style={{fontSize:10,color:PRI,marginLeft:4}}>â© cruza aÃ±o</span>}</span><span style={{fontSize:13,color:MUT}}>{g.total}d</span><button onClick={()=>{setEditGid(g.gid);setForm({tipo:g.tipo,ini:s0.ini,fin:sN.fin});setMsgOk("");setMsgErr("");}} style={{background:"none",border:"none",color:"#6a9fd8",cursor:"pointer",fontSize:18}}>âï¸</button><button onClick={()=>eliminar(empSel,g.gid)} style={{background:"none",border:"none",color:"#b94a4a",cursor:"pointer",fontSize:18}}>â</button></div>;})}
      </Card>}
    </div></div>;
  }
