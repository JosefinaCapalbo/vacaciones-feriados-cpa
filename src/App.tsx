...
export default function App(){
  ...
  const [empPassErr,setEmpPassErr]=useState(false);
  ...
  function ingresarEmp(){
    const claveReq=(cfg[empSel]||{}).clave;
    if(claveReq){
      if(empPass===claveReq){setVista("emp");reset();setEmpPass("");setEmpPassErr(false);}
      else setEmpPassErr(true);
    }else{setVista("emp");reset();}
  }
  ...
  function eliminarEmpleado(nombre){
    if(!window.confirm("¿Eliminar a "+nombre+"? Se va a borrar también su historial guardado. Esta acción no se puede deshacer."))return;
    const nuevosEmps=emps.filter(e=>e!==nombre);
    const nuevoCfg={...cfg};delete nuevoCfg[nombre];
    const nuevosDatos={...datos};delete nuevosDatos[nombre];
    save(nuevosEmps,nuevoCfg,nuevosDatos);
    if(adminEmp===nombre){setAdminEmp(nuevosEmps[0]||"");resetAdmin();}
  }
  ...
  if(vista==="login")return(<div style={page}><div style={{maxWidth:460,margin:"0 auto"}}>
    ...
    {loginTab==="emp"&&<><Card title="Acceso empleado"><label style={{fontSize:14,color:MUT,display:"block",marginBottom:6}}>Tu nombre</label><select value={empSel} onChange={e=>{setEmpSel(e.target.value);setEmpPass("");setEmpPassErr(false);}} style={inp}><option value="">-- Elegir --</option>{[...visibles].sort((a,b)=>a.localeCompare(b,"es")).map(e=><option key={e}>{e}</option>)}</select>{empSel&&(cfg[empSel]||{}).clave&&<div style={{marginTop:8}}><label style={{fontSize:14,color:MUT,display:"block",marginBottom:6}}>Clave</label><input type="password" value={empPass} placeholder="Tu clave" onChange={e=>{setEmpPass(e.target.value);setEmpPassErr(false);}} onKeyDown={e=>{if(e.key==="Enter")ingresarEmp();}} style={inp}/>{empPassErr&&<p style={{color:"#b94a4a",fontSize:13,margin:"4px 0"}}>Clave incorrecta.</p>}</div>}<Btn onClick={ingresarEmp} disabled={!empSel}>Ingresar</Btn></Card>
    ...
    {loginTab==="cal"&&<Card><CalendarioPublico emps={emps.filter(e=>e.trim().toLowerCase()!=="admin")} datos={datos}/></Card>}
  </div></div>);
  ...
  return <div style={page}><div style={{maxWidth:680,margin:"0 auto"}}>
    ...
    {adminTab==="gestion"&&<div style={{display:"grid",gap:12}}>
      ...
      <Card title="👤 Configurar empleados"><div style={{display:"grid",gap:8}}>{[...emps].sort((a,b)=>a.localeCompare(b,"es")).map((e,i)=>{const c=cfg[e]||{},mV=c.mv!=null?c.mv:MAX_DEF,mF=c.mf!=null?c.mf:MAX_DEF,oc=c.oculto||false,clv=c.clave||"";return <div key={e} style={{background:BG,borderRadius:10,padding:"12px",border:"1px solid "+BDR,opacity:oc?0.65:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}><div style={{width:30,height:30,borderRadius:"50%",background:AVA[i%AVA.length],display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:11,color:TXT,flexShrink:0}}>{ini2(e)}</div><span style={{fontWeight:600,color:TXT,fontSize:14,flex:1}}>{e}</span><button onClick={()=>{const nc={...cfg,[e]:{...(cfg[e]||{}),oculto:!oc}};save(null,nc,null);}} style={{background:oc?"#d4f0c0":"#fadadd",border:"none",borderRadius:8,padding:"4px 10px",fontSize:12,cursor:"pointer",color:oc?"#2d6a1f":"#b94a4a",fontWeight:600}}>{oc?"👁 Mostrar":"🙈 Ocultar"}</button><button onClick={()=>eliminarEmpleado(e)} title="Eliminar empleado" style={{background:"none",border:"none",color:"#b94a4a",cursor:"pointer",fontSize:18,marginLeft:4}}>🗑️</button></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}><div><label style={{fontSize:12,color:MUT,display:"block",marginBottom:3}}>🌴 Días vac./año</label><input type="number" min="0" max="365" value={mV} onChange={ev=>{const nc={...cfg,[e]:{...(cfg[e]||{}),mv:parseInt(ev.target.value)||0}};save(null,nc,null);}} style={{...inp,marginBottom:0}}/></div><div><label style={{fontSize:12,color:MUT,display:"block",marginBottom:3}}>🎉 Días fer./año</label><input type="number" min="0" max="365" value={mF} onChange={ev=>{const nc={...cfg,[e]:{...(cfg[e]||{}),mf:parseInt(ev.target.value)||0}};save(null,nc,null);}} style={{...inp,marginBottom:0}}/></div></div><div><label style={{fontSize:12,color:MUT,display:"block",marginBottom:3}}>🔒 Clave de acceso (opcional)</label><input type="text" value={clv} placeholder="Sin clave — acceso libre" onChange={ev=>{const nc={...cfg,[e]:{...(cfg[e]||{}),clave:ev.target.value}};save(null,nc,null);}} style={{...inp,marginBottom:0}}/></div></div>;})}</div></Card>
    </div>}
  </div></div>;
}